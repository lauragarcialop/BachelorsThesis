...
def syn_current(duration=100, t_on=10, t_off=90, dt=0.01, n_pulses=1, delay=0, seed=None):
    ...
    I_inj = torch.zeros_like(t)
    
    for k in range(n_pulses):
        I_inj[int(np.round((t_on + k*delay)/dt)):int(np.round((t_off + k*delay)/dt))] = 1
...
def run_HHModel(params, I, t, dt, A_soma, initial_conditions = False, V0=0, m0=0, h0=0, n0=0, nois_fact=1e-6, seed=None):
    ...
    I_app = params[3].to(torch.float64)  # muA/cm2
    I_inj = I_app * I
    ...
    solver = Solver(F, t, dt, method='RK2', D=D)

    X = solver.solve(X0, U=I_inj)

    return dict({"V" : X[0], "m" : X[1], "h" : X[2], "n" : X[3], "I_inj" : I_inj, "t" : t, "A" : A_soma})
