...
def run_LOModel(params, t, dt, Dx=0, Dy=0, method='RK4', seed=None):
    lda, omega = params[0].to(torch.float64), 1
    a, b = 1, params[1].to(torch.float64)
...
def plot_trajectory(sim, params=None):
...
def find_level_set_constants(params):
    ...
    omega, a = torch.ones(params.shape[0]), torch.ones(params.shape[0])
